...
echo "$($PSQL "SELECT DISTINCT t.name FROM games g JOIN teams t ON g.winner_id = t.team_id WHERE g.year = 2014 AND g.round = 'Eighth-Final' 
              UNION 
              SELECT DISTINCT t.name FROM games g JOIN teams t ON g.opponent_id = t.team_id WHERE g.year = 2014 AND g.round = 'Eighth-Final'")"
